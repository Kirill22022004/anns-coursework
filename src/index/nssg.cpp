...
std::vector<std::vector<int>> build_nssg(const std::vector<std::vector<float>>& data, const std::vector<std::vector<int>>& knn_graph, int l = 40, int m = 20) {
    int N = data.size();
    ...
    for (int v = 0; v < N; ++v) {
        for (int u : nsg[v]) {
            if (find(nsg[u].begin(), nsg[u].end(), v) == nsg[u].end()) {
                nsg[u].push_back(v);
            }
        }
    }
    std::vector<std::vector<int>> nssg(N);
    for (int v = 0; v < N; v++) {
        std::vector<int> E = nsg[v];
        std::sort(E.begin(), E.end());
        E.erase(std::unique(E.begin(), E.end()), E.end());
        std::sort(E.begin(), E.end(), [&](int a, int b) {
            return euclidean_distance(data[v], data[a]) < euclidean_distance(data[v], data[b]);
        ...
        std::vector<int> R;
        R.push_back(E[0]);
        for (size_t i = 1; i < E.size() && R.size() < (size_t)2 * m; i++) {
            int p = E[i];
            if (!has_conflict(data, v, p, R)) {
                R.push_back(p);
            }
    ...
    while (1) {
        ...
        nssg[i].push_back(best_j);
        nssg[best_j].push_back(i);
    }
    return nssg;
}
...
int main() {
    ...
    std::vector<std::vector<int>> nssg = build_nssg(data, knn_graph);
    save_graph(nssg, "nssg.txt");
    std::cout << "NSSG built and saved." << std::endl;
    return 0;
}
